import{i as m}from"./api.CuXibZx2.js";function s(n){const a=document.createElement("div");return a.textContent=n||"",a.innerHTML.replace(/"/g,"&quot;").replace(/'/g,"&#39;")}async function u(){try{let n=function(e){if(!e.winner)return`
        ...
        `};const a=await m();if(a.status!=="success")throw new Error(a.message||"API error");const r=a.data,i=document.getElementById("results-intro");if(!r.published){i.textContent=r.message||"Results have not been published yet. Check back after December 17, 2026 at 10:00 AM EAT.",container.innerHTML=`
          <div class="text-center py-16">
            <div class="text-6xl mb-6">🗳️</div>
            <h2 class="text-heading-3 text-ink mb-4">Voting in Progress</h2>
            <p class="text-body-md text-steel max-w-xl mx-auto">
              Results will be announced on <strong>December 17, 2026 at 10:00 AM EAT</strong>.
              In the meantime, browse categories and cast your vote!
            </p>
            <a href="/awards" class="btn btn-primary mt-6">Browse Categories</a>
          </div>
        `;return}const l=r.results||[];if(i.textContent="Congratulations to all the outstanding nominees and winners! The results were announced on December 17, 2026 at the gala in Nairobi.",!l.length){document.getElementById("results-all-content").innerHTML='<div class="text-center text-steel py-12">No results available.</div>',document.getElementById("results-winners-content").innerHTML='<div class="text-center text-steel py-12">No winners yet.</div>';return}const c=l.reduce((e,t)=>e+(t.total_votes||0),0),d=l.filter(e=>e.winner);document.getElementById("results-all-content").innerHTML=l.map(n).join(""),document.getElementById("results-winners-content").innerHTML=d.length?d.map(n).join(""):'<div class="text-center text-steel py-8">All categories have recorded votes.</div>',document.getElementById("summary-stats").classList.remove("hidden"),document.getElementById("sum-categories").textContent=l.length,document.getElementById("sum-votes").textContent=c.toLocaleString()}catch(n){console.error("Results load error:",n),document.getElementById("results-intro").textContent="Unable to load results. Please try again later.",document.getElementById("results-all-content").innerHTML='<div class="text-center text-brand-error py-12">Failed to load results.</div>'}}u();
