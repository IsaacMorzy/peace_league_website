const l=window.ENV?.PUBLIC_API_URL||"",c={"peace-education":"https://images.unsplash.com/photo-1497633762265-9d179a990aa6?w=600&q=80&auto=format&fit=crop","youth-empowerment":"https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=600&q=80&auto=format&fit=crop","community-reconciliation":"https://images.unsplash.com/photo-1559027615-cd4628902d4a?w=600&q=80&auto=format&fit=crop",health:"https://images.unsplash.com/photo-1571019613454-1cb2f99b2d8b?w=600&q=80&auto=format&fit=crop",water:"https://images.unsplash.com/photo-1534274988757-a28bf1a57c17?w=600&q=80&auto=format&fit=crop",agriculture:"https://images.unsplash.com/photo-1500937386664-56d1dfef3854?w=600&q=80&auto=format&fit=crop",training:"https://images.unsplash.com/photo-1524178232363-1fb2b075b655?w=600&q=80&auto=format&fit=crop",event:"https://images.unsplash.com/photo-1540575467063-178a50c2df87?w=600&q=80&auto=format&fit=crop"},i=new IntersectionObserver(o=>{o.forEach(a=>{if(a.isIntersecting){const e=a.target,s=parseFloat(e.dataset.progressWidth)||0;requestAnimationFrame(()=>{e.style.width=s+"%"}),i.unobserve(e)}})},{threshold:.3});function d(){document.querySelectorAll(".progress-bar").forEach(o=>i.observe(o))}async function m(){try{const o=await fetch(`${l}/api/method/peace_league_website.api.get_causes`);if(!o.ok)return;const a=await o.json(),e=a.message||a;if(e.status==="success"&&e.data&&e.data.length>0){const s=document.getElementById("causes-list");s.innerHTML=e.data.map((t,r)=>{const n=t.image||c[t.name?.toLowerCase()]||"";return`
            <div class="bg-canvas rounded-2xl border border-hairline overflow-hidden shadow-[0_4px_16px_rgba(0,0,0,0.04)] transition-all duration-300 hover:shadow-lg hover:-translate-y-2 animate-in fade-in slide-in-from-bottom-4 duration-700 img-zoom" style="animation-delay: ${r*100}ms">
              ...
              <div class="p-6">
                <span class="inline-flex items-center px-2 py-0.5 rounded-sm text-xs font-semibold mb-3" style="background-color: rgba(55,114,207,0.15); color: #3772cf;">${t.category||"Cause"}</span>
